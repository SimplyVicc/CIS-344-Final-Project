...
class RestaurantDatabase:
    ...
    def addReservation(self, customer_id, name, email, reservation_time, number_of_guests, special_requests):
        ''' Method to insert a new reservation using the stored procedure '''
        if self.connection.is_connected():
            cursor = self.connection.cursor()
            try:
                # Prepare the call to the stored procedure
                args = [customer_id, name, email, reservation_time, number_of_guests, special_requests]
                result_args = cursor.callproc('addReservation', args)
                
                self.connection.commit()
                print("Reservation added successfully for customer ID:", result_args[0])
            except Error as e:
